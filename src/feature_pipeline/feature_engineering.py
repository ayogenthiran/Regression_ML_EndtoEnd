...
def frequency_encode(train: pd.DataFrame, eval_df: pd.DataFrame, col: str):
    freq_map = train[col].value_counts()
    train[f"{col}_freq"] = train[col].map(freq_map)
    eval_df[f"{col}_freq"] = eval_df[col].map(freq_map).fillna(0)
    return train, eval_df, freq_map


#Uses target encoding (replace category with average of target variable).
#Fitted only on train (prevents leakage).
def target_encode(train: pd.DataFrame, eval_df: pd.DataFrame, col: str, target: str):
    """
    ...
    eval_df[encoded_col] = te.transform(eval_df[col])
    return train, eval_df, te



def drop_unused_columns(train: pd.DataFrame, eval_df: pd.DataFrame):
