...
def test_frequency_encode_counts_values():
    ...
    train, eval_df, freq_map = frequency_encode(train, eval_df, "zipcode")
    assert train["zipcode_freq"].tolist() == [2, 2, 1]
    assert eval_df["zipcode_freq"].tolist() == [2, 0]
...
def test_target_encode_applies_mapping():
    ...
    eval_df = pd.DataFrame({"city_full": ["A", "B"]})
    train, eval_df, te = target_encode(train, eval_df, "city_full", "price")
    assert "city_full_encoded" in train.columns
    assert eval_df["city_full_encoded"].notnull().all()
